...
def train_and_test_load(class_name,train_loader,test_loader,num_classes,length,FCNmodel,index):
    ...
    #print(index,len(FCNmodel.conv1_bias))
    ...
    for epoch in range(epoches):
        for images, labels in train_loader:
            ...
            loss =0.9*loss_func(outputlist[-1], labels) +0.1*( net.SquareLossEW(outputlist[0],premodelist[0])+net.SquareLossEW(outputlist[1],premodelist[1])+
            net.SquareLossEW(outputlist[2],premodelist[2])+net.SquareLossEW(outputlist[3],premodelist[3])+net.SquareLossEW(outputlist[0],premodelist2[0])+net.SquareLossEW(outputlist[1],premodelist2[1])+
            net.SquareLossEW(outputlist[2],premodelist2[2])+net.SquareLossEW(outputlist[3],premodelist2[3]))
            optimizer.zero_grad()
            loss.backward()  
            optimizer.step()
            del images,labels    
...
f.writelines("FedTESAVGOneTeacher Task-------0.1------------0.9---1\n")
f.close()
print("FedTESAVG Task------------1")
avg = 0    
   ...
   # f = open(logTxt,mode="a+")
    ...
    if i == 0:
        ...
        #del model_state
         ...
         sota,model_stateL = train_and_test(classname,train_loader,test_loader,num_classes,length)
