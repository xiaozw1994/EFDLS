...
def train_and_test_load(class_name,train_loader,test_loader,num_classes,length,FCNmodel):
    epoches = 500  # 
    lr = 0.0001  # 
    input_num = 1
    output_num =  num_classes
    ...
    for epoch in range(epoches):
        for images, labels in train_loader:
            ...
            loss = loss_func(output, labels)+ FCNmodel.ComputeEWS(model)
            optimizer.zero_grad()
            loss.backward()  
            optimizer.step()  
        correct = 0
...
numbers = len(names)
logTxt = "FedNewAVG.txt"
f = open(logTxt,mode="a+")
f.writelines("FedAVG Task------------1\n")
f.close()
print("FedAVG Task------------1")
avg = 0    
...
for e in range(1,Fed_iteration):
    ...
    for i in range(len(names)):
        ...
        print("Load---------dataset:",classname)
        if i == 0:
            sota,model_stateP = train_and_test_load(classname,train_loader,test_loader,num_classes,length,FCNmodel)
            shared = net.StoreParameters()
            shared.Appended(model_stateP)
            #del model_stateP
        else:
            sota,model_stateV = train_and_test_load(classname,train_loader,test_loader,num_classes,length,FCNmodel)
